...
export class BattlemapEditorComponent implements OnInit, OnChanges, AfterViewInit, OnDestroy {
  ...
  loadPreviewSvg(url: string): void {
    fetch(url)
      ...
      .then(svgContent => {
        ...
        this.rawSvgContent = svgContent.replace(/<!DOCTYPE[^>]*>/i, '').trim();
        
        // Process and update the display
        this.updatePreviewSvg();
      })
      .catch(error => {
  ...
  updateSvgContainer(): void {
    // Process and update the SVG display
    this.updatePreviewSvg();
  }
  
  /**
   * Process raw SVG content (add grid/highlight overlays) and update previewSvgContent
   */
  private updatePreviewSvg(): void {
    ...
    if (this.selectedObjectId !== null) {
      processedSvg = this.addHighlightToSvg(processedSvg, this.selectedObjectId);
    }
    
    // Bypass security since we control the backend and need SVG filters/masks to work
    // The SVG is generated by our own backend code, so it's safe
    this.previewSvgContent = this.sanitizer.bypassSecurityTrustHtml(processedSvg);
  }
  ...
  private generateWater(
    ...
    if (config.edgeSide) {
      ...
      switch (config.edgeSide) {
        case 'bottom':
          // Water at the bottom edge - simple rectangle filling
          // Fill a rectangle from the bottom row up to waterDepth rows
